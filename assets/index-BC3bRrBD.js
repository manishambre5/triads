(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))c(o);new MutationObserver(o=>{for(const n of o)if(n.type==="childList")for(const i of n.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&c(i)}).observe(document,{childList:!0,subtree:!0});function s(o){const n={};return o.integrity&&(n.integrity=o.integrity),o.referrerPolicy&&(n.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?n.credentials="include":o.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function c(o){if(o.ep)return;o.ep=!0;const n=s(o);fetch(o.href,n)}})();let a=4;function d(e){a>0&&(a--,h()),a===0&&(console.log("Game Over"),p(e))}function h(){const e=document.getElementById("counter");e.textContent="close".repeat(a)}function p(e){const t=document.querySelector(".triads");document.querySelector(".wordgrid").innerHTML="",document.querySelector(".mistakes").classList.add("text-transparent"),document.querySelector(".buttons").innerHTML=`
  ...
  `).join("")}function f(e){const t=document.querySelector(".wordgrid");t&&(t.innerHTML=u(e))}function x(e){const t=document.querySelector(".triads");t&&(t.classList.replace("hidden","flex"),t.querySelectorAll("div").forEach(c=>c.classList.remove("animate-exist")),t.innerHTML+=`
      ...
      ">
        <div class="font-bold">${e[0].connection}</div>
        <div class="italic">${e.map(c=>c.word).join(", ")}</div>
      </div>
  ...
  `,document.getElementById("shuffle").addEventListener("click",()=>{f(l([...r]))}),document.getElementById("submit").addEventListener("click",v)}document.querySelector(".wordgrid").addEventListener("click",e=>{const t=document.querySelectorAll(".selected");e.target.classList.contains("word")&&(e.target.classList.contains("selected")?e.target.classList.remove("selected"):t.length<3?e.target.classList.add("selected"):(t.forEach(s=>s.classList.add("animate-shake")),setTimeout(()=>{t.forEach(s=>s.classList.remove("animate-shake"))},500)))});function v(){const e=new Audio("complete.mp3"),t=Array.from(document.querySelectorAll(".selected")),s=Array.from(document.querySelectorAll(".selected")).map(n=>n.dataset.word),c=s.map(n=>{const i=r.find(m=>m.word===n);return i?i.connectionIndex:-1}),o=new Set(c).size===1;if(t.length===3)if(o){let n=r.filter(i=>s.includes(i.word));return r=r.filter(i=>!s.includes(i.word)),r.length===0&&(console.log("Well Done!"),document.querySelector(".wordgrid").classList.replace("grid","hidden"),setTimeout(()=>{e.volume=.5,e.play().catch(i=>{console.warn("Audio autoplay blocked:",i)}),document.querySelector(".congrats").classList.replace("hidden","absolute")},500)),x(n),f(r),!0}else return t.forEach(n=>n.classList.add("animate-shake")),setTimeout(()=>{t.forEach(n=>n.classList.remove("animate-shake"))},500),t.forEach(n=>n.classList.remove("selected")),d(r),!1;else return t.forEach(n=>n.classList.add("animate-shake")),setTimeout(()=>{t.forEach(n=>n.classList.remove("animate-shake"))},500),t.forEach(n=>n.classList.remove("selected")),!1}d(r);
